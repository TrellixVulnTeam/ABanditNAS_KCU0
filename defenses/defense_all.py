...
parser.add_argument('--initial_const', help='initial value of the constant c', type=float, default=0.01)
...
def main():
    ...
    if args.data == 'cifar10':
        attack_type = ['clean', 'mi_fgsm']
        attackers_val = [None]
        for i in attack_type:
            if i != 'clean':
                # attacker = attackers[i](model_attack, eps=args.eps, nb_iter=args.nb_iter, eps_iter=args.eps_iter)
                attacker = attackers[i](model_attack, loss_fn=test_criterion, eps=args.eps, nb_iter=args.nb_iter,\
                    eps_iter=args.eps_iter, num_classes=n_classes, initial_const=args.initial_const)
